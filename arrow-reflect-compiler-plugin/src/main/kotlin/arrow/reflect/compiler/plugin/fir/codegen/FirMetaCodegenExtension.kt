...
import arrow.meta.FirMetaNestedGenerationContext
...
class FirMetaCodegenExtension(
  ...
  private fun metaContext(context: MemberGenerationContext?): FirMetaContext =
    FirMetaMemberGenerationContext(templateCompiler, session, context)

  private fun metaNestedContext(context: NestedClassGenerationContext?): FirMetaContext =
    FirMetaNestedGenerationContext(templateCompiler, session, context)
    ...
    return if (!templateCompiler.compiling) {
      val firClass: FirClass? =
        invokeMeta(
          ...
          metaContext(null),
          emptyList(),
          Meta.Generate.TopLevel.Class::class,
          "classes",
          classId
        )
  ...
  fun nestedClasses(classSymbol: FirClassSymbol<*>): Set<Name>? =
    // if (classSymbol.isMetaAnnotated()) // TODO() can't lookup annotations on this phases are they
    // are not resolved
    invokeMeta(
  ...
  override fun getNestedClassifiersNames(
    classSymbol: FirClassSymbol<*>,
    context: NestedClassGenerationContext
  ): Set<Name> = nestedClasses(classSymbol).orEmpty()
  ...
  fun topLevelFunctions(): Set<CallableId>? =
    ...
    invokeMeta(
      ...
      metaContext(null),
      emptyList(),
      Meta.Generate.TopLevel.Functions::class,
      "functions"
    )
  ...
  fun topLevelProperties(): Set<CallableId>? =
    invokeMeta(
      ...
      metaContext(null),
      emptyList(),
      Meta.Generate.TopLevel.Properties::class,
      "properties"
    )
  ...
  val metaAnnotatedPredicate: DeclarationPredicate
    get() =
      DeclarationPredicate.create { metaAnnotated(listOf(AnnotationFqn("arrow.meta.Meta")), false) }
